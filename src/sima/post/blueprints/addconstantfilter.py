...
class AddConstantFilterBlueprint(OperationNodeBlueprint):
    ...
    def __init__(self, name="AddConstantFilter", package_path="sima/post", description=""):
        ...
        self.add_attribute(Attribute("constant","number","Constant value to subtract",default=0.0))
        self.add_attribute(EnumAttribute("axis","sima/post/SignalAxis","Select the value from the x-axis or the y-axis"))
        self.add_attribute(Attribute("shiftAxis","boolean","Shift the x or y axis so that the first index is 0",default=False))