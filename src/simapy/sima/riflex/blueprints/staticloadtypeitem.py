...
class StaticLoadTypeItemBlueprint(MOAOBlueprint):
    ...
    def __init__(self, name="StaticLoadTypeItem", package_path="sima/riflex", description=""):
        ...
        self.add_attribute(Attribute("accuracy","number","Required accuracy measured by displacement norm",default=1e-06))
        self.add_attribute(EnumAttribute("convergenceNorm","sima/riflex/ConvergenceNorm",""))
        self.add_attribute(Attribute("energyAccuracy","number"," Required accuracy measured by energy norm. Value is not used if convergence norm is 'Displacement'.",default=1e-06))
        self.add_attribute(Attribute("entered","boolean","start condition for pipe-in-pipe contact\nEntered should be used for analysis of slender structures such as risers, cables and umbilicals\nNot Entered is intended to be used for marine operations",default=True))
        ...
        self.add_attribute(Attribute("growthFactor","number","Scaling factor for growth profile for all lines",default=1.0))
        self.add_attribute(BlueprintAttribute("marineGrowthScalings","sima/riflex/MarineGrowthScaling","",True,Dimension("*")))
