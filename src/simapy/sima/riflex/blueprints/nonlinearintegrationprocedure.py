...
class NonLinearIntegrationProcedureBlueprint(MOAOBlueprint):
    ...
    def __init__(self, name="NonLinearIntegrationProcedure", package_path="sima/riflex", description=""):
        ...
        self.add_attribute(Attribute("equilibriumIterationAccuracy","number","Desired accuracy for equilibrium iteration measured by a modified euclidian displacement norm (norm of squared translations). Recommended values: 10e-6 - 10e-5.",default=1e-06))
        self.add_attribute(Attribute("energyAccuracy","number","Required accuracy measured by energy norm. This value is relevant only if Convergence Norm is 'Both'.",default=1e-06))
